...
// The hardware limits of the sensors.
...
// Three states for overall vehicle health.
typedef enum
{
    STATE_NORMAL = 0,
    STATE_DEGRADED = 1,
...
typedef enum
{
    ...
    FAULT_BIT_HIGH_TEMP = 4,
    FAULT_BIT_COUNT = 5
} FaultBit;

typedef enum
{
    FAULT_SEVERITY_MAJOR = 0, // Degraded state
    FAULT_SEVERITY_CRITICAL = 1 // Forces fault mode and directly to safe state
} FaultSeverity;
...
typedef struct
{
    ...
    uint32_t cycle_count; // 1 cycle per millisecond
    ...
    // This flag indicates whether the system needs to be reset due to a critical fault.
