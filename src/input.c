...
void validate_inputs(VehicleInput *input, VehicleStatus *status){
    ...
    if (input->gear >= MIN_GEAR && input->gear <= MAX_GEAR){
        ...
        input->gear_valid   = 0;
        /* Do NOT restore last good gear here; invalid gear must still
           be visible to the control layer so it can raise a fault.
           We only preserve for display purposes after fault is raised. */
    }
